...
class TopicAnalyzer:
    ...
    def apply_context_priority(self, topic: str, keyword_matches: List[str], phrase_matches: List[str]) -> float:
        ...
        # Các quy tắc thưởng được đọc từ TOPIC_CONFIG["context_boost"] (domain/topic_config.py)
        # → Không cần hardcode ở đây, chỉ cần cập nhật config là đủ
        ...
        # Lấy cấu hình thưởng ngữ cảnh của chủ đề từ TOPIC_CONFIG
        # Nếu chủ đề chưa có "context_boost" → trả về None, bỏ qua

        if not config:
            return boost
            # Chủ đề không có cấu hình thưởng → trả về 0.0

        # --- Thưởng loại 1: Cụm từ đặc trưng cao ---
        for rule in config.get("phrase_bonus", []):
            # Mỗi rule là {"phrases": [...], "score": x}
            # Nếu BẤT KỲ cụm từ nào trong rule khớp → cộng điểm thưởng
            if any(p in phrase_set for p in rule.get("phrases", [])):
                boost += rule.get("score", 0.0)

        # --- Thưởng loại 2: Tổ hợp từ khóa ---
        for rule in config.get("keyword_combo", []):
            # Mỗi rule là {"keywords": [...], "score": x}
            # Chỉ cộng điểm khi TẤT CẢ keyword trong combo cùng xuất hiện
